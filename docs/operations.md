...
`granite-parse-yaml-mark` is centered on parsers work in Ruby. The technical target is: Implement a Ruby parsers project for yaml policy evaluation, using deny and allow fixtures and explainable decision traces.
...
The score is deterministic. The current threshold is 172. Risk has a penalty of 4, latency has a penalty of 4, and weight contributes a bonus of 3.
